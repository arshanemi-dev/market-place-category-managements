...
// The form is its own component so it mounts fresh every time the dialog opens: the input starts
// with the right value already in it, so autofocus can select the whole name (type to replace it).
function NameForm({ initialValue, placeholder, onSubmit, onClose }) {
...
export default function NameDialog({ open, title, initialValue = '', placeholder = 'Category name', onSubmit, onClose }) {
  return (
    <Modal open={open} onClose={onClose} title={title}>
      <NameForm initialValue={initialValue} placeholder={placeholder} onSubmit={onSubmit} onClose={onClose} />
    </Modal>
  )
}
